...
void Renderer::UploadPerFrameProperties(Shader* shader)
{
    ...
    for (auto& shaderProperty : activeProperties)
    {
        if (shaderProperty == ShaderBasicProperties::LightTransform && !lights.empty())
        {
            ...
            auto lightTransform = UniformValue{activeLight->GetEntity()->transform.GetAllTransformMatrix()};
        ...
        {
            auto activeLightEntity = lights[0];
            auto lightPosition = UniformValue{activeLightEntity->transform.m_position};
...
void Renderer::UploadPerModelProperties(Shader* shader, const ModelRenderInfo* object)
{
    ...
    {
        auto modelTransformMatrix = UniformValue{entity->transform.GetAllTransformMatrix()};
    ...
    {
        auto normalMatrix = entity->transform.GetNormalMatrix();
...
void Renderer::UploadLightProperties(Shader* shader)
{
    ...
    for (int i = 0; i < sceneLights.size(); i++)
    {
        ...
        auto positionValue = UniformValue{sceneLight->transform.m_position};
        ...
        auto innerValue = UniformValue{lightComponent->m_innerConeAngle};
        ...
        auto outerValue = UniformValue{lightComponent->m_outerConeAngle};
        ...
        shader->setUniformPerName(attLinearUniform, attQuadraticValue);
